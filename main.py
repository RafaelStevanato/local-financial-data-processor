...
import pandas as pd
from src.config import DATA_FILE, OUTPUT_DIR, REPORT_FILE, REQUIRED_COLUMNS
from src.validator import validate_file_exists
...
def load_data(file_path: Path) -> pd.DataFrame:
    ...
    if df["amount"].isna().any():
        raise ValueError("Invalid values found in AMOUNT column")
...
def generate_report(metrics: dict, summary: pd.DataFrame) -> None:
    lines = [
